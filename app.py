...
if "inventory_filter" not in st.session_state:
    st.session_state["inventory_filter"] = "전체"
...
def process_uploaded_files(files_dict):
    ...
    # 배차 원본 컬럼을 별도로 보관한다.
    # 이후 출고파일 매핑 컬럼(예: 출고예정_매핑)이 추가되더라도
    # 배송유형 판정은 반드시 배차파일 원본의 예정일자를 우선 사용한다.
    dispatch_source_columns = list(df_b.columns)
    ...
    # [수정] 출고 파일(df_c)의 출고예정(예정일자) 및 출고일시를 배차 파일(df_b)로 매핑
    c_plan_col = None
    c_time_col = None
    if df_c is not None:
        for col in df_c.columns:
            if any(k in str(col).strip() for k in ["출고예정", "예정일자", "예정일", "배차예정", "출고예정일", "배송예정", "배송예정일", "납기예정", "납기일"]):
                c_plan_col = col
            if any(k in str(col).strip() for k in ["출고일시", "출고시간", "일시"]):
                c_time_col = col

        if c_plan_col:
            p_map12 = df_c.set_index("key12")[c_plan_col].to_dict() if "key12" in df_c.columns else {}
            p_map10 = df_c.set_index("key10")[c_plan_col].to_dict() if "key10" in df_c.columns else {}
            p_map_addr = df_c.groupby("key_addr")[c_plan_col].first().to_dict() if "key_addr" in df_c.columns else {}
            
            df_b["출고예정_매핑"] = df_b["key12"].map(p_map12).fillna(df_b["key10"].map(p_map10)).fillna(df_b["key_addr"].map(p_map_addr))
        else:
            df_b["출고예정_매핑"] = pd.NaT

        if c_time_col:
            t_map12 = df_c.set_index("key12")[c_time_col].to_dict() if "key12" in df_c.columns else {}
            t_map10 = df_c.set_index("key10")[c_time_col].to_dict() if "key10" in df_c.columns else {}
            t_map_addr = df_c.groupby("key_addr")[c_time_col].first().to_dict() if "key_addr" in df_c.columns else {}
            
            df_b["출고일시_매핑"] = df_b["key12"].map(t_map12).fillna(df_b["key10"].map(t_map10)).fillna(df_b["key_addr"].map(t_map_addr))
        else:
            df_b["출고일시_매핑"] = pd.NaT

    # 피킹조회 매칭용 일련번호 정규화
    # 공백/숨은 공백/엑셀 숫자형의 .0 차이를 제거해 매칭 실패를 줄인다.
    def normalize_serial(series):
        return (
            series.fillna("")
            .astype(str)
            .str.strip()
    ...
    df_b["serial_key"] = normalize_serial(df_b["일련번호"])
    df_b["serial_prefix15"] = df_b["serial_key"].str[:15]
    df_b["피킹매칭상태"] = "피킹정보 없음"

    if df_p is not None and "일련번호" in df_p.columns:
        df_p["serial_key"] = normalize_serial(df_p["일련번호"])
        df_p["serial_prefix15"] = df_p["serial_key"].str[:15]

        # 15자리 보정 매칭은 피킹조회에서 해당 prefix가 단 하나의 일련번호를
        # 가리키는 경우에만 허용해 잘못된 매칭을 방지한다.
        prefix_serial_count = (
            df_p[df_p["serial_prefix15"].str.len() >= 12]
            .groupby("serial_prefix15")["serial_key"]
            .nunique()
        )
        unique_prefixes = set(prefix_serial_count[prefix_serial_count == 1].index)

        exact_keys = set(df_p["serial_key"])
        exact_match = df_b["serial_key"].isin(exact_keys) & df_b["serial_key"].ne("")
        fallback_match = (
            ~exact_match
            & df_b["serial_prefix15"].str.len().ge(12)
            & df_b["serial_prefix15"].isin(unique_prefixes)
        )

        df_b.loc[exact_match, "피킹매칭상태"] = "일치"
        df_b.loc[fallback_match, "피킹매칭상태"] = "보정일치"

        unique_prefix_df = df_p[df_p["serial_prefix15"].isin(unique_prefixes)].copy()

        def map_picking_column(column_name):
            if column_name not in df_p.columns:
                return pd.Series(np.nan, index=df_b.index)

            exact_map = df_p.groupby("serial_key")[column_name].first().to_dict()
            prefix_map = unique_prefix_df.groupby("serial_prefix15")[column_name].first().to_dict()

            result = df_b["serial_key"].map(exact_map)
            fallback_values = df_b["serial_prefix15"].map(prefix_map)

            # exact 일련번호가 없는 행에 대해서만 안전한 15자리 보정값 사용
            result = result.where(~fallback_match, fallback_values)
            return result

        df_b["로케이션_표시"] = map_picking_column("로케이션")
        df_b["디오티_표시"] = map_picking_column("DOT")
        df_b["LI_표시"] = map_picking_column("LI")
        df_b["SS_표시"] = map_picking_column("SS")
    ...
    # [중요] 내일물량 판정용 예정일자는 '배차파일 원본 컬럼'에서만 찾는다.
    # 출고파일에서 추가된 출고예정_매핑 같은 컬럼이 먼저 잡히는 것을 방지한다.
    def find_dispatch_col(keywords):
        # 정확한 컬럼명을 먼저 우선 탐색
        for keyword in keywords:
            for col in dispatch_source_columns:
                if str(col).strip() == keyword:
                    return col
        # 정확히 일치하지 않을 때만 부분 일치
        for keyword in keywords:
            for col in dispatch_source_columns:
                if keyword in str(col).strip():
                    return col
    ...
    plan_date_col = find_dispatch_col([
        "예정일자", "출고예정일자", "출고예정일", "배차예정일자", "배차예정일",
        "배송예정일자", "배송예정일", "납기예정일자", "납기예정일", "납기일"
    ])

    # BC는 피킹조회 매칭 여부와 무관하게 배차 데이터의 PTTN으로 직접 판정한다.
    # 지정된 패턴이면 Y, 그 외는 - 로 표시한다.
    BC_PATTERNS = {
        "TURANZA EV",
        "POTENZA SPORT AS",
        "DESTINATION LE3",
    ...
    if pttn_col:
        ...
        df["BC_표시"] = np.where(pttn_norm.isin(BC_PATTERNS), "Y", "-")
    else:
        df["BC_표시"] = "-"

    # 세부사항 표시용 가용재고수량:
    # 재고 파일의 동일 품목코드 + 동일 로케이션을 기준으로만 매칭하고,
    # DOT까지 일치하는 재고가 있으면 해당 값을 우선 사용한다.
    # 품목코드만 같은 전체 재고 합계로는 폴백하지 않는다.
    df["가용재고수량_표시"] = np.nan

    if df_i is not None and not df_i.empty and code_col:
        def find_inventory_col(keywords):
            for keyword in keywords:
        ...
        inv_code_col = find_inventory_col(["품목코드", "상품코드", "자재코드", "SKU"])
        inv_loc_col = find_inventory_col(["입고로케이션", "로케이션"])
        inv_dot_col = find_inventory_col(["DOT"])
        inv_avail_col = find_inventory_col(["가용재고수량", "가용재고"])

        if inv_code_col and inv_loc_col and inv_avail_col and "로케이션_표시" in df.columns:
            inv_work = df_i.copy()
            ...
            inv_work["_code_key"] = normalize_key(inv_work[inv_code_col])
            inv_work["_loc_key"] = normalize_key(inv_work[inv_loc_col])
            inv_work["_avail_qty"] = pd.to_numeric(inv_work[inv_avail_col], errors="coerce").fillna(0)

            df_code_key = normalize_key(df[code_col])
            df_loc_key = normalize_key(df["로케이션_표시"])
            mapped_avail = pd.Series(np.nan, index=df.index, dtype="float64")

            if inv_dot_col and "디오티_표시" in df.columns:
                inv_work["_dot_key"] = normalize_key(inv_work[inv_dot_col])
                df_dot_key = normalize_key(df["디오티_표시"])
                inv_map_3 = inv_work.groupby(
                    ["_code_key", "_loc_key", "_dot_key"], dropna=False
                )["_avail_qty"].sum().to_dict()
                key_3 = pd.Series(
                    list(zip(df_code_key, df_loc_key, df_dot_key)),
                    index=df.index
                )
                mapped_avail = key_3.map(inv_map_3)

            inv_map_2 = inv_work.groupby(
                ["_code_key", "_loc_key"], dropna=False
            )["_avail_qty"].sum().to_dict()
            key_2 = pd.Series(list(zip(df_code_key, df_loc_key)), index=df.index)
            mapped_avail = mapped_avail.fillna(key_2.map(inv_map_2))

            df["가용재고수량_표시"] = mapped_avail

    df["가용재고수량_표시"] = pd.to_numeric(
        df["가용재고수량_표시"], errors="coerce"
    ).fillna(0)
    ...
    # [수정] 내일물량은 배차파일의 예정일자를 최우선 기준으로 판정한다.
    # 예정일자가 내일이면 출고일시/차량번호/기사 입력 여부와 관계없이 무조건 내일물량이다.
    kst_today = datetime.now(timezone(timedelta(hours=9))).date()
    tomorrow_date = kst_today + timedelta(days=1)
    tomorrow_ts = pd.Timestamp(tomorrow_date)

    def parse_excel_date_series(series):
        """엑셀 날짜 직렬값, YYYYMMDD, 일반 날짜문자열을 날짜 단위로 안전하게 변환."""
        result = pd.Series(pd.NaT, index=series.index, dtype="datetime64[ns]")
        raw = series.copy()

        # 1) 엑셀 날짜 직렬값(대략 1954~2119 범위) 처리
        numeric = pd.to_numeric(raw, errors="coerce")
        excel_serial_mask = numeric.between(20000, 80000, inclusive="both")
        ...
        if remaining.any():
            ...
            # YYYYMMDD 형태를 명시적으로 처리
            yyyymmdd_mask = text_date.str.fullmatch(r"\d{8}", na=False)
            if yyyymmdd_mask.any():
                idx = text_date.index[yyyymmdd_mask]
                result.loc[idx] = pd.to_datetime(
                    text_date.loc[idx], format="%Y%m%d", errors="coerce"
                )

            # 그 외 날짜/일시 문자열 처리
            general_idx = text_date.index[~yyyymmdd_mask]
            if len(general_idx) > 0:
                try:
                    parsed = pd.to_datetime(
                        text_date.loc[general_idx], errors="coerce", format="mixed"
                    )
                except TypeError:
                    parsed = pd.to_datetime(text_date.loc[general_idx], errors="coerce")
                result.loc[general_idx] = parsed

        return pd.to_datetime(result, errors="coerce").dt.normalize()

    # 배차파일 예정일자가 있으면 그것이 최우선.
    if plan_date_col:
        plan_date_parsed = parse_excel_date_series(df[plan_date_col])
        is_tomorrow = plan_date_parsed.eq(tomorrow_ts)
    else:
        plan_date_parsed = pd.Series(pd.NaT, index=df.index, dtype="datetime64[ns]")
        is_tomorrow = pd.Series(False, index=df.index)

    # 배차 예정일자가 비어 있는 행만 기존 출고일시 기준을 보조적으로 사용한다.
    if time_col:
        time_parsed_b = parse_excel_date_series(df[time_col])
        if plan_date_col:
            is_tomorrow = is_tomorrow | (plan_date_parsed.isna() & time_parsed_b.eq(tomorrow_ts))
        else:
            is_tomorrow = time_parsed_b.eq(tomorrow_ts)
    else:
        time_parsed_b = pd.Series(pd.NaT, index=df.index, dtype="datetime64[ns]")

    gyeongsang_kw = ["부산", "대구", "울산", "경남", "경북", "경상", "밀양", "양산", "창원", "김해", "포항", "경주"]
    ...
    # 택배 판정: 차량번호가 "경동택배"이고 택배구분이 "택배"인 경우
    # (기사명 컬럼이 비어 있어도 정상 처리)
    if car_col:
        is_courier = (
            df[car_col].fillna("").astype(str).str.strip().eq("경동택배")
            & s_courier.eq("택배")
        )
    else:
        is_courier = pd.Series(False, index=df.index)
...
def render_top_nav():
    ...
    with col1:
        if st.button("🏠 홈", key="top_btn_home", use_container_width=True):
            st.session_state["page_stack"] = ["home_category"]
            st.rerun()
    with col2:
        if st.button("📦 재고조회", key="top_btn_inventory", use_container_width=True):
            st.session_state["selected_category"] = "재고조회"
            st.session_state["page_stack"] = ["home_category", "inventory_view"]
            st.rerun()
    with col3:
        is_disabled = len(st.session_state["page_stack"]) <= 1
        if st.button("↩ 뒤로가기", key="top_btn_back", use_container_width=True, disabled=is_disabled):
            if not is_disabled:
...
def render_bottom_nav():
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("🏠 홈", key="bot_btn_home", use_container_width=True):
            st.session_state["page_stack"] = ["home_category"]
            st.rerun()
    with col2:
        if st.button("📦 재고조회", key="bot_btn_inventory", use_container_width=True):
            st.session_state["selected_category"] = "재고조회"
            st.session_state["page_stack"] = ["home_category", "inventory_view"]
            st.rerun()
    with col3:
        is_disabled = len(st.session_state["page_stack"]) <= 1
        if st.button("↩ 뒤로가기", key="bot_btn_back", use_container_width=True, disabled=is_disabled):
            if not is_disabled:
                st.session_state["page_stack"].pop()
                st.rerun()
...
# 재고조회 스크롤 위치 유지
# ==========================================
def keep_inventory_scroll_position():
    """재고조회 위젯 rerun 후에도 사용자가 보던 세로 스크롤 위치를 복원한다."""
    components.html(
        """
        <script>
        (function () {
            try {
                ...
                const scrollKey = "__inventory_scroll_y__";
                const handlerKey = "__inventory_scroll_handler__";

                // Streamlit rerun 직후 이전 위치로 복원
                if (typeof p[scrollKey] === "number") {
                    const restoreY = p[scrollKey];
                    setTimeout(() => p.scrollTo(0, restoreY), 20);
                    setTimeout(() => p.scrollTo(0, restoreY), 120);
                }

                // rerun마다 이벤트 리스너가 중복되지 않게 기존 핸들러 제거
                if (p[handlerKey]) {
                    p.removeEventListener("scroll", p[handlerKey]);
                }

                p[handlerKey] = function () {
                    p[scrollKey] = p.scrollY || p.document.documentElement.scrollTop || 0;
                };
                p.addEventListener("scroll", p[handlerKey], { passive: true });
            } catch (e) {
                // 브라우저 보안정책 등으로 접근이 막히면 기본 Streamlit 동작 사용
            }
        })();
        </script>
        """,
...
def render_main_app():
    ...
    # --- 일반 통합 재고조회 뷰 화면 ---
        ...
        # 검색/필터 변경으로 Streamlit이 rerun되어도 현재 스크롤 위치를 유지
        keep_inventory_scroll_position()
        ...
        if df_i is None or df_i.empty:
            ...
            # 재고이동 파일 컬럼 찾기
            c_loc = find_inv_col(["입고로케이션", "로케이션"])
            c_code = find_inv_col(["품목코드", "상품코드"])
            c_size = find_inv_col(["SIZE", "사이즈"])
            c_pttn = find_inv_col(["PTTN", "패턴"])
            c_dot = find_inv_col(["DOT"])
            c_avail = find_inv_col(["가용재고수량", "가용재고"])
            c_total = find_inv_col(["전체재고수량", "전체재고", "총재고"])
            c_item_type = find_inv_col(["품목구분"])
            c_tire_type = find_inv_col(["타이어구분"])
            c_storage_note = find_inv_col(["적치비고"])
            c_date = find_inv_col(["입고일자"])
            c_container = find_inv_col(["컨테이너번호"])

            # 검색창 바로 위 재고 분류 버튼
            # 분류 기준: 재고이동 파일의 타이어구분 + PTTN
            filter_cols = st.columns(4)
            filter_buttons = [
                ("전체", "📋 전체", "inv_filter_all"),
                ("승용", "🚗 승용", "inv_filter_psr"),
                ("화물", "🚚 화물", "inv_filter_tbr"),
                ("마케팅", "🎁 마케팅", "inv_filter_mkt"),
            ]
            def set_inventory_filter(filter_value):
                st.session_state["inventory_filter"] = filter_value

            for col_obj, (filter_value, button_label, button_key) in zip(filter_cols, filter_buttons):
                with col_obj:
                    st.button(
                        button_label,
                        use_container_width=True,
                        key=button_key,
                        type="primary" if st.session_state["inventory_filter"] == filter_value else "secondary",
                        on_click=set_inventory_filter,
                        args=(filter_value,),
                    )

            tire_type_s = (
                inv_df[c_tire_type].fillna("").astype(str).str.strip().str.upper()
                if c_tire_type else pd.Series("", index=inv_df.index)
            )
            pttn_s = (
                inv_df[c_pttn].fillna("").astype(str).str.strip().str.upper()
                if c_pttn else pd.Series("", index=inv_df.index)
            )

            selected_inv_filter = st.session_state.get("inventory_filter", "전체")
            if selected_inv_filter == "승용":
                inv_df = inv_df[tire_type_s.eq("PSR")].copy()
            elif selected_inv_filter == "화물":
                inv_df = inv_df[tire_type_s.eq("TBR") | pttn_s.str.contains("TUBE", na=False)].copy()
            elif selected_inv_filter == "마케팅":
                inv_df = inv_df[tire_type_s.str.contains("마케팅|골프공", na=False)].copy()

            search_inv = st.text_input(
                "🔍 재고 검색어 입력 (상품코드 / SIZE / PTTN / 로케이션)",
                "",
                key="inv_search_box"
            )
            ...
            if search_inv:
                search_mask = pd.Series(False, index=inv_df.index)
                if c_code:
                    search_mask = search_mask | inv_df[c_code].astype(str).str.contains(search_inv, case=False, na=False)
                if c_size:
                    search_mask = search_mask | inv_df[c_size].astype(str).str.contains(search_inv, case=False, na=False)
                if c_pttn:
                    search_mask = search_mask | inv_df[c_pttn].astype(str).str.contains(search_inv, case=False, na=False)
                if c_loc:
                    search_mask = search_mask | inv_df[c_loc].astype(str).str.contains(search_inv, case=False, na=False)
                inv_df = inv_df[search_mask].copy()

            def format_dot_value(value):
                if pd.isna(value):
                    return "-"
                s = str(value).strip()
                ...
                if s.endswith(".0"):
                    s = s[:-2]
                # 숫자 DOT는 엑셀 화면처럼 4자리 유지 (예: 26 -> 0026)
                if s.isdigit():
                    return s.zfill(4)
                return s

            # 엑셀 예시 화면과 같은 컬럼 순서
            display_inv = pd.DataFrame(index=inv_df.index)
            if c_loc: display_inv["로케이션"] = inv_df[c_loc].fillna("-")
            ...
            if c_dot: display_inv["DOT"] = inv_df[c_dot].apply(format_dot_value)
            if c_size: display_inv["SIZE"] = inv_df[c_size].fillna("-")
            if c_pttn: display_inv["PTTN"] = inv_df[c_pttn].fillna("-")
            if c_code: display_inv["상품코드"] = inv_df[c_code].fillna("-")
            ...
            def highlight_inventory(data):
                ...
                shade = "background-color: rgba(220, 235, 250, 0.75);"
                if "가용재고수량" in styles.columns:
                    styles["가용재고수량"] = shade
                if "DOT" in styles.columns:
                    styles["DOT"] = shade
                return styles
    ...
    # --- 3단계: 목록 현황 ---
        ...
        if sub == "택배":
            ...
            # 당/익/미배차 목록에서는 점포 행 선택 시 바로 세부사항(피킹리스트)으로 이동.
            # Streamlit 기본 dataframe에는 더블클릭 전용 이벤트가 없어
            # 행 선택 이벤트를 사용하며, 더블클릭해도 첫 클릭에서 바로 열립니다.
            detail_select_subs = ["당", "익", "미배차"]
            direct_selection_supported = False
            if sub in detail_select_subs:
                try:
                    detail_table_key = f"detail_store_table_{st.session_state['detail_table_nonce']}"
                    ...
                    direct_selection_supported = True
                    ...
                    if selected_rows:
                        ...
                        if 0 <= selected_idx < len(show_df):
                            row_data = show_df.iloc[selected_idx]
                    ...
                    # 구버전 Streamlit에서는 기존 표/선택 방식으로 자동 폴백
                    st.dataframe(styled_df, use_container_width=True, hide_index=True, height=550)
            else:
                st.dataframe(styled_df, use_container_width=True, hide_index=True, height=550)

            # 당/익/미배차의 최신 Streamlit에서는 행 선택으로 바로 진입하므로 기존 선택 UI는 숨김.
            # 그 외 화면 또는 구버전 Streamlit에서는 기존 선택 방식을 그대로 유지.
            if not (sub in detail_select_subs and direct_selection_supported):
                st.write("---")
    ...
    # --- 4단계: 피킹리스트 ---
        ...
        if row_data is not None:
            ...
            matching_items["가용재고수량_표시"] = pd.to_numeric(
                matching_items["가용재고수량_표시"], errors="coerce"
            ).fillna(0).round(0).astype(int)

            missing_pick_count = int(
                (matching_items["피킹매칭상태"] == "피킹정보 없음").sum()
            )
            corrected_pick_count = int(
                (matching_items["피킹매칭상태"] == "보정일치").sum()
            )

            if missing_pick_count > 0:
                st.warning(
                    f"⚠️ 피킹조회와 일련번호가 매칭되지 않은 품목이 {missing_pick_count}건 있습니다. "
                    "해당 행은 '피킹정보 없음'으로 표시됩니다."
                )
            if corrected_pick_count > 0:
                st.caption(
                    f"ℹ️ 일련번호 형식 차이를 보정해 연결한 품목이 {corrected_pick_count}건 있습니다."
                )

            detail_display = pd.DataFrame({
                "로케이션": matching_items["로케이션_표시"],
                "피킹정보": matching_items["피킹매칭상태"],
                "출고수량": matching_items["수량_표시"],
                "가용재고수량": matching_items["가용재고수량_표시"],
                "DOT": matching_items["디오티_표시"],
                "BC": matching_items["BC_표시"].fillna("-"),
                "품목코드": matching_items["품목코드_표시"],
                "SIZE": matching_items["사이즈_표시"],
                "PTTN": matching_items["패턴_표시"],
