...
def process_uploaded_files(files_dict):
    ...
    if df_p is not None and "일련번호" in df_p.columns:
        ...
        # 세부사항 표시용 BC(바코드품목) 컬럼이 피킹 파일에 있으면 함께 매핑
        p_bc_col = None
        for col in df_p.columns:
            col_text = str(col).strip()
            col_upper = col_text.upper()
            if p_bc_col is None and (col_upper == "BC" or "바코드품목" in col_text or "바코드" in col_text):
                p_bc_col = col

        bc_map = df_p.groupby("serial_key")[p_bc_col].first().to_dict() if p_bc_col else {}
        ...
        df_b["BC_표시"] = df_b["serial_key"].map(bc_map) if bc_map else np.nan
    ...
    # 세부사항 표시용 BC(바코드품목) 값 보완
    if "BC_표시" not in df.columns:
        df["BC_표시"] = np.nan
    ...
    for col in df.columns:
        col_text = str(col).strip()
        col_upper = col_text.upper()
        if col_upper == "BC" or "바코드품목" in col_text or "바코드" in col_text:
            bc_col = col
            break

    if bc_col:
        df["BC_표시"] = df["BC_표시"].fillna(df[bc_col])
    ...
    # --- 4단계: 세부사항 뷰 ---
        ...
        if row_data is not None:
            ...
            detail_display = pd.DataFrame({
                ...
                "BC(바코드품목)": matching_items["BC_표시"].fillna("-"),
