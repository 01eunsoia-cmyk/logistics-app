...
# 버튼 글씨체 크기 2배 확대 및 굵기 적용을 위한 글로벌 CSS
st.markdown("""
    <style>
    div.stButton > button {
...
def render_bottom_nav():
    ...
    if st.session_state["authenticated"]:
        st.write("")
        if st.button("로그아웃", key="bot_logout_btn", use_container_width=True):
            st.session_state["authenticated"] = False
            st.session_state["role"] = None
            st.rerun()
...
def render_main_app():
    ...
    if current_page == "home_category":
        ...
        with col1:
            if st.button("승용", use_container_width=True, key="cat_psr"):
                st.session_state["selected_category"] = "승용"
                st.session_state["page_stack"].append("sub_category")
                st.rerun()
        with col2:
            if st.button("화물", use_container_width=True, key="cat_tbr"):
                st.session_state["selected_category"] = "화물"
                st.session_state["page_stack"].append("sub_category")
                st.rerun()
        with col3:
            if st.button("마케팅", use_container_width=True, key="cat_mkt"):
                st.session_state["selected_category"] = "마케팅"
                st.session_state["page_stack"].append("sub_category")
                st.rerun()
        with col4:
            if st.button("재고조회", use_container_width=True, key="cat_inventory"):
    ...
    # --- 2단계: 배송 유형 선택 ---
        ...
        unit_unit = "개" if cat == "마케팅" else "본"
        
        # 접두사 설정 (승용 또는 화물)
        prefix_str = f"{cat} " if cat in ["승용", "화물"] else ""

        if cat == "마케팅":
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                if st.button(f"당착\n\n{qty_dang:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "당"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col2:
                if st.button(f"익착\n\n{qty_ik:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "익"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col3:
                if st.button(f"택배\n\n{qty_courier:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "택배"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col4:
                if st.button("재고조회", use_container_width=True):
                    st.session_state["page_stack"].append("mkt_inventory_view")
                    st.rerun()
        else:
            col1, col2, col3, col4, col5 = st.columns(5)
            with col1:
                if st.button(f"{prefix_str}당착\n\n{qty_dang:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "당"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col2:
                if st.button(f"{prefix_str}익착\n\n{qty_ik:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "익"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col3:
                if st.button(f"{prefix_str}미배차\n\n{qty_unassigned:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "미배차"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col4:
                if st.button(f"{prefix_str}택배\n\n{qty_courier:,} {unit_unit}", use_container_width=True):
                    st.session_state["selected_sub"] = "택배"
                    st.session_state["page_stack"].append("list_view")
                    st.rerun()
            with col5:
                if st.button(f"{prefix_str}내일물량\n\n{qty_tomorrow:,} {unit_unit}", use_container_width=True):
    ...
    # --- 3단계: 목록 현황 ---
        ...
        if sub == "택배":
            st.metric("택배 총 수량", f"{total_qty:,} {unit_unit}")
            st.write("---")
            ...
            m_col1.metric("전체 총수량", f"{total_qty:,} {unit_unit}")
            m_col2.metric("출력 완료 수량 (○)", f"{printed_qty:,} {unit_unit}")
            m_col3.metric("미출력 수량 (X)", f"{unprinted_qty:,} {unit_unit}")
