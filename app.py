...
def process_uploaded_files(files_dict):
    ...
    # 수정: 패턴(PTTN)이나 규격에 'TUBE'가 있는 경우 마케팅이 아닌 TBR(화물) 재고로 분류
    def map_mkt(row):
        ...
        # 패턴 또는 규격에 TUBE가 포함되면 무조건 TBR(화물)
...
def render_header(data_time_str):
    ...
    st.markdown(f"🕒 **최근 업데이트 일자:** `{display_time}`")
...
# ==========================================
# 3. 로그인 및 모드 분리 화면 (수정: 일반모드가 첫 번째 배치)
# ==========================================
def render_login():
    st.title("🚚 실시간 물량집계 시스템")
    st.subheader("접속 모드를 선택하고 비밀번호를 입력해주세요.")

    # 수정사항 1: 일반 모드를 관리자 모드보다 앞에 배치
...
def render_main_app():
    ...
    # --- 2단계: 당착 / 익착 / 택배 / 미배차 선택 (수정: 상단 총수량 표기) ---
    ...
    # --- 3단계: 목록 현황 (수정: 차량번호=점포명=주소 같으면 수량 합산 취합, ○/X 표기, 넘버삭제) ---
        ...
        # 동일한 출력여부, 차량번호, 점포명, 주소에 대해 수량 취합(Groupby)
        ...
        # 수정사항 2: hide_index=True 적용으로 맨 앞 인덱스 숫자 제거
        event = st.dataframe(
            show_df,
