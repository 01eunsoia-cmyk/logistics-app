...
def render_main_app():
    ...
    # --- 피킹조회 뷰 (안전형 변환 적용) ---
        ...
        if df_p is None or df_p.empty:
            ...
            pick_df = df_p.copy()
            # PyArrow 직렬화 오류 방지를 위해 모든 컬럼을 안전하게 문자열/숫자로 변환
            for col in pick_df.columns:
                if pick_df[col].dtype == object or str(pick_df[col].dtype).startswith("datetime"):
                    pick_df[col] = pick_df[col].fillna("-").astype(str)
        ...
        if row_data is not None:
            ...
            if df_p is not None and not df_p.empty:
                ...
                if not p_match.empty:
                    ...
                    detail_display["로케이션"] = p_match[c_loc].fillna("-").astype(str) if c_loc else "-"
                    ...
                    detail_display["상품코드"] = p_match[c_code].fillna("-").astype(str) if c_code else "-"
                    detail_display["사이즈"] = p_match[c_size].fillna("-").astype(str) if c_size else "-"
                    detail_display["LI"] = p_match[c_li].fillna("-").astype(str) if c_li else "-"
                    detail_display["SS"] = p_match[c_ss].fillna("-").astype(str) if c_ss else "-"
                    detail_display["품목비고"] = p_match[c_note].fillna("-").astype(str) if c_note else "-"
                    ...
                    st.warning("피킹조회 데이터에서 해당 점포의 일치하는 상세 품목을 찾지 못했습니다.")
            else:
