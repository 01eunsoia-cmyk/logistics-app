...
def process_uploaded_files(files_dict):
    ...
    for name, file_obj in files_dict.items():
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                if "피킹" in file_text or "피킹" in sheet_text:
                    df_p = temp_df
                elif "출고" in file_text or "출고" in sheet_text:
                    if df_c is None:
                        df_c = temp_df
    ...
    if df_p is not None:
        ...
        if p_serial_col:
            df_p["serial_key"] = df_p[p_serial_col].astype(str).str.strip()
            df_b["serial_key"] = df_b["일련번호"].astype(str).str.strip()

            p_cols = {}
            for col in df_p.columns:
                col_str = str(col).strip().upper()
                if "로케이션" in col_str or "LOCATION" in col_str or "LOC" in col_str:
                    p_cols["LOC"] = col
                elif "DOT" in col_str or "디오티" in col_str or "주차" in col_str:
                    p_cols["DOT"] = col
                elif col_str == "LI" or "하중" in col_str:
                    p_cols["LI"] = col
                elif col_str == "SS" or "속도" in col_str:
                    p_cols["SS"] = col

            for target_key, p_col in p_cols.items():
                p_map = df_p.groupby("serial_key")[p_col].first().to_dict()
                df_b[f"피킹_{target_key}"] = df_b["serial_key"].map(p_map)
    ...
    # 피킹조회 매칭 결과 적용 (없으면 기본값 -)
    df["로케이션_표시"] = df.get("피킹_LOC", pd.Series("-", index=df.index)).fillna("-")
    df["디오티_표시"] = df.get("피킹_DOT", pd.Series("-", index=df.index)).fillna("-")
    df["LI_표시"] = df.get("피킹_LI", pd.Series("-", index=df.index)).fillna("-")
    df["SS_표시"] = df.get("피킹_SS", pd.Series("-", index=df.index)).fillna("-")
...
def render_main_app():
    ...
    # --- 3단계: 목록 현황 ---
        ...
        show_df = grouped_df[["출력상태", "차량번호_표시", "점포명_표시", "수량_표시", "주소_표시"]].rename(columns=renamed_cols)
    ...
    # --- 4단계: 세부사항 뷰 ---
        ...
        if row_data is not None:
            st.info(f"**점포명:** {row_data['점포명']} | **차량번호:** {row_data['차량번호']} | **주소:** {row_data['주소']}")

            matching_items = df[
                (df["점포명_표시"] == row_data["점포명"]) &
                (df["주소_표시"] == row_data["주소"]) &
                (df["차량번호_표시"] == row_data["차량번호"])
            ].copy()
            ...
            detail_display = detail_display.sort_values(by="로케이션", ascending=False)
