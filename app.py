...
def process_uploaded_files(files_dict):
    ...
    # 추가 항목 검색
    loc_col = find_col(["로케이션", "LOCATION", "렉위치", "위치", "LOC"])
    dot_col = find_col(["디오티", "DOT", "주차"])
    li_col = find_col(["LI", "하중지수", "LOAD"])
    ss_col = find_col(["SS", "속도기호", "SPEED"])
    ...
    # 디테일 화면용 컬럼 매핑
    df["로케이션_표시"] = df[loc_col].fillna("-") if loc_col else "-"
    df["디오티_표시"] = df[dot_col].fillna("-") if dot_col else "-"
    df["LI_표시"] = df[li_col].fillna("-") if li_col else "-"
    df["SS_표시"] = df[ss_col].fillna("-") if ss_col else "-"
...
def render_main_app():
    ...
    # --- 4단계: 세부사항 뷰 (수정 완료) ---
        ...
        if row_data is not None:
            ...
            # 원본 열 존재 여부 체크 및 매핑
            loc_val = "로케이션_표시" if "로케이션_표시" in matching_items.columns else "상품코드_표시"
            dot_val = "디오티_표시" if "디오티_표시" in matching_items.columns else "상품코드_표시"
            li_val = "LI_표시" if "LI_표시" in matching_items.columns else "상품코드_표시"
            ss_val = "SS_표시" if "SS_표시" in matching_items.columns else "상품코드_표시"

            # 요청받은 열 순서: 로케이션 > 수량 > 디오티 > 패턴 > 사이즈 > LI > SS (구분 열 삭제)
            detail_display = matching_items[[loc_val, "수량_표시", dot_val, "패턴_표시", "사이즈_표시", li_val, ss_val]].rename(
                columns={
                    loc_val: "로케이션",
                    "수량_표시": "수량",
                    dot_val: "디오티",
