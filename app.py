...
def process_uploaded_files(files_dict):
    ...
    # [수정] 내일물량 최우선 판별 로직 (datetime vs date 비교 타입 에러 해결)
    ...
    plan_date_parsed_b = pd.to_datetime(df[plan_date_col], errors="coerce").dt.normalize() if plan_date_col else pd.Series(pd.NaT, index=df.index)
    time_parsed_b = pd.to_datetime(df[time_col], errors="coerce").dt.normalize() if time_col else pd.Series(pd.NaT, index=df.index)

    plan_date_parsed_c = pd.to_datetime(df["출고예정_매핑"], errors="coerce").dt.normalize() if "출고예정_매핑" in df.columns else pd.Series(pd.NaT, index=df.index)
    time_parsed_c = pd.to_datetime(df["출고일시_매핑"], errors="coerce").dt.normalize() if "출고일시_매핑" in df.columns else pd.Series(pd.NaT, index=df.index)

    is_plan_tomorrow = (plan_date_parsed_b >= tomorrow_ts) | (plan_date_parsed_c >= tomorrow_ts)
    is_time_tomorrow = (time_parsed_b >= tomorrow_ts) | (time_parsed_c >= tomorrow_ts)
