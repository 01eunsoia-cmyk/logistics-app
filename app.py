...
def render_login():
    ...
    st.caption("출고, 배차, 피킹 데이터를 업로드하여 통합 집계합니다.")
    st.markdown("")
...
# =========================================================
# 핵심 집계 로직 (출고 + 배차 + 피킹 통합)
# =========================================================
def process_logistics_data(df_c, df_b, df_p=None):
...
st.markdown(
    "출고, 배차, 피킹 파일을 모두 업로드하면 원하시는 기준대로 자동 분류 및 집계됩니다."
)
...
with col3:
    picking_file = st.file_uploader("피킹조회 엑셀 파일 업로드", type=["xlsx", "xls"])

if chuko_file and batcha_file:
    try:
        df_c = pd.read_excel(chuko_file)
        df_b = pd.read_excel(batcha_file)
        df_p = pd.read_excel(picking_file) if picking_file else None

        df_filtered, grouped = process_logistics_data(df_c, df_b, df_p)
    ...
    st.info(
        "💡 출고 파일과 배차 파일(선택: 피킹 파일)을 업로드해주시면 즉시 집계됩니다."
    )
