...
def process_uploaded_files(files_dict):
    ...
    if df_p is not None and "일련번호" in df_p.columns:
        ...
        # 세부사항 표시용 BC(바코드품목) / 가용재고수량 컬럼이 피킹 파일에 있으면 함께 매핑
        p_bc_col = None
        p_avail_col = None
        for col in df_p.columns:
            ...
            if p_bc_col is None and (col_upper == "BC" or "바코드품목" in col_text or "바코드" in col_text):
                p_bc_col = col
            if p_avail_col is None and ("가용재고수량" in col_text or "가용재고" in col_text):
                p_avail_col = col

        bc_map = df_p.groupby("serial_key")[p_bc_col].first().to_dict() if p_bc_col else {}
        avail_map = df_p.groupby("serial_key")[p_avail_col].first().to_dict() if p_avail_col else {}
        ...
        df_b["가용재고수량_표시"] = df_b["serial_key"].map(avail_map) if avail_map else np.nan
    ...
    # 세부사항 표시용 가용재고수량:
    # 피킹 파일 값이 있으면 우선 사용하고, 없으면 재고 파일을
    # 품목코드+로케이션+DOT → 품목코드+로케이션 → 품목코드 순으로 매칭
    if "가용재고수량_표시" not in df.columns:
        df["가용재고수량_표시"] = np.nan
    ...
    if df_i is not None and not df_i.empty and code_col:
        ...
        if inv_code_col and inv_avail_col:
            inv_work = df_i.copy()
            ...
            if inv_loc_col and "로케이션_표시" in df.columns:
            ...
            inv_map_1 = inv_work.groupby("_code_key")["_avail_qty"].sum().to_dict()
            mapped_avail = mapped_avail.fillna(df_code_key.map(inv_map_1))
