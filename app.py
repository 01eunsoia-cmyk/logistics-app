...
def process_uploaded_files(files_dict):
    ...
    # [수정] 내일물량 최우선 판별 로직 (원본 및 매핑된 날짜 모두 검토)
    kst_today = datetime.now(timezone(timedelta(hours=9))).date()
    tomorrow_date = kst_today + timedelta(days=1)

    # df 원본 컬럼 확인
    plan_date_parsed_b = pd.to_datetime(df[plan_date_col], errors="coerce").dt.date if plan_date_col else pd.Series(pd.NaT, index=df.index)
    time_parsed_b = pd.to_datetime(df[time_col], errors="coerce").dt.date if time_col else pd.Series(pd.NaT, index=df.index)

    # 매핑된 컬럼 확인
    plan_date_parsed_c = pd.to_datetime(df["출고예정_매핑"], errors="coerce").dt.date if "출고예정_매핑" in df.columns else pd.Series(pd.NaT, index=df.index)
    time_parsed_c = pd.to_datetime(df["출고일시_매핑"], errors="coerce").dt.date if "출고일시_매핑" in df.columns else pd.Series(pd.NaT, index=df.index)

    # 한 군데라도 내일 이후의 날짜가 기록되어 있으면 내일물량으로 판단
    is_plan_tomorrow = (plan_date_parsed_b >= tomorrow_date) | (plan_date_parsed_c >= tomorrow_date)
    is_time_tomorrow = (time_parsed_b >= tomorrow_date) | (time_parsed_c >= tomorrow_date)
