...
def render_main_app():
    ...
    # --- 일반 통합 재고조회 뷰 화면 (품목구분이 불량품인 경우 최상단 정렬, 전체재고수량과 입고일자 사이에 품목구분, 적치비고 배치) ---
        ...
        if df_i is None or df_i.empty:
            ...
            search_inv = st.text_input("🔍 재고 검색어 입력 (품목코드 / SIZE / PTTN / 입고로케이션)", "")
            inv_df = df_i.copy()
            ...
            # 품목구분이 불량품인 경우 리스트 최상단으로 올리기 위한 정렬 처리
            if c_item_type:
    ...
    # --- 마케팅 전용 재고조회 뷰 화면 (재고파일 안의 품목구분 사용) ---
        ...
        if df_i is None or df_i.empty:
            ...
            search_mkt_inv = st.text_input("🔍 마케팅 재고 검색어 입력 (품목구분 / 상품코드 / SIZE)", "")
            if search_mkt_inv:
                def find_c(names):
                    for n in names:
                        for col in inv_df.columns:
            ...
            st.dataframe(styled_mkt_inv, use_container_width=True, hide_index=True)
    ...
    # --- 3단계: 목록 현황 ---
        ...
        if sub == "택배":
            ...
            st.dataframe(styled_show_df, use_container_width=True, hide_index=True)
