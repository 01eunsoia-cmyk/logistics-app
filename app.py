...
def process_uploaded_files(files_dict):
    ...
    # 출력여부 ○ / X 표기
    df["출력상태"] = np.where(df[print_col].astype(str).str.upper().str.contains("Y"), "○", "  ") if print_col else "  "
    ...
    processed_data = {
        "updated_at": get_kst_now(),  # 서울 시간 적용
        "raw_df": df
    }
...
def render_main_app():
    ...
    # --- 3단계: 목록 현황 (수정: 상단 총수량 메트릭, ○/X 매핑 보장) ---
        ...
        # 기존 pkl 데이터 상에 '출력'/'미출력' 텍스트가 남아있더라도 '○'/'X'로 강제 치환
        sub_df["출력상태"] = sub_df["출력상태"].replace({"출력": "○", "미출력": "X"})

        # 상단 핵심 메트릭 (총 수량 현황 표시)
