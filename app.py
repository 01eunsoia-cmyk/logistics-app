...
def render_main_app():
    ...
    # --- 4단계: 세부사항 뷰 (거래처명 및 주소 기준 연동) ---
        ...
        if row_data is not None:
            ...
            # 배차 데이터(df) 및 피킹조회(df_p)에서 동일한 점포명(거래처명)과 배송처주소를 가진 세부 행들을 정확히 추출
            match_cond = (
            ...
            # 피킹조회 데이터가 있는 경우, 거래처명 + 주소 기준으로 피킹조회의 세부 품목/로케이션/DOT/LI/SS 병합
            if df_p is not None and not df_p.empty:
                df_p_temp = df_p.copy()
                df_p_temp["p_key"] = df_p_temp["출고처명(거래처)"].astype(str).str.strip() + "_" + df_p_temp["배송처주소"].astype(str).str.strip()
                matching_items["p_key"] = matching_items["점포명_표시"].astype(str).str.strip() + "_" + matching_items["주소_표시"].astype(str).str.strip()

                # 피킹 파일의 상세 컬럼들을 매핑
                for col in ["로케이션", "DOT", "LI", "SS", "품목코드", "SIZE", "PTTN", "품목비고"]:
                    if col in df_p_temp.columns:
                        p_map = df_p_temp.groupby("p_key")[col].first().to_dict()
                        if col == "로케이션":
                            matching_items["로케이션_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("로케이션_표시", "-"))
                        elif col == "DOT":
                            matching_items["디오티_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("디오티_표시", "-"))
                        elif col == "LI":
                            matching_items["LI_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("LI_표시", "-"))
                        elif col == "SS":
                            matching_items["SS_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("SS_표시", "-"))
                        elif col in ["품목코드", "상품코드"]:
                            matching_items["상품코드_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("상품코드_표시", "-"))
                        elif col == "SIZE":
                            matching_items["사이즈_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("사이즈_표시", "-"))
                        elif col == "PTTN":
                            matching_items["패턴_표시"] = matching_items["p_key"].map(p_map).fillna(matching_items.get("패턴_표시", "-"))
            ...
            for col_name, default_val in [
                ...
                if col_name not in matching_items.columns:
                    matching_items[col_name] = default_val
