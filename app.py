...
def render_main_app():
    ...
    # --- 피킹조회 탭 화면 신설 ---
        ...
        if df_p is None or df_p.empty:
            ...
            # 출력여부 매칭 (출고 데이터 기반 또는 피킹 데이터 내 필드 활용)
            # 출고 df를 활용해 출력상태(○/X) 가져오기
            print_map = df.set_index("serial_key")["출력상태"].to_dict() if "serial_key" in df.columns else {}
            if "일련번호" in pick_df.columns:
                pick_df["serial_key"] = pick_df["일련번호"].astype(str).str.strip()
                pick_df["출력여부"] = pick_df["serial_key"].map(print_map).fillna("X")
            else:
                pick_df["출력여부"] = "X"

            # 당/익 구분 매칭 (배송유형 매칭)
            type_map = df.set_index("serial_key")["배송유형"].to_dict() if "serial_key" in df.columns else {}
            if "일련번호" in pick_df.columns:
                pick_df["배송유형"] = pick_df["serial_key"].map(type_map).fillna("익")
            else:
                pick_df["배송유형"] = "익"

            # 컬럼 매핑 확인
            def find_p_col(candidates):
                for c in candidates:
                    for col in pick_df.columns:
                        if c in str(col):
                            return col
                return None

            c_loc = find_p_col(["로케이션"])
            c_code = find_p_col(["품목코드", "상품코드", "코드"])
            c_size = find_p_col(["SIZE", "사이즈"])
            c_pttn = find_p_col(["PTTN", "패턴"])
            c_store = find_p_col(["출고처명(거래처)", "점포명", "거래처"])
            c_addr = find_p_col(["배송처주소", "주소"])

            display_pick = pd.DataFrame()
            display_pick["출력여부"] = pick_df["출력여부"]
            display_pick["당익"] = pick_df["배송유형"]
            display_pick["로케이션"] = pick_df[c_loc].fillna("-") if c_loc else "-"
            display_pick["상품코드"] = pick_df[c_code].fillna("-") if c_code else "-"
            display_pick["SIZE"] = pick_df[c_size].fillna("-") if c_size else "-"
            display_pick["PTTN"] = pick_df[c_pttn].fillna("-") if c_pttn else "-"
            display_pick["점포명"] = pick_df[c_store].fillna("-") if c_store else "-"
            display_pick["주소"] = pick_df[c_addr].fillna("-") if c_addr else "-"

            # 검색어 필터
            search_pick = st.text_input("🔍 피킹조회 검색어 입력 (로케이션 / 상품코드 / 점포명 / 주소)", "", key="pick_search_box")
            if search_pick:
                display_pick = display_pick[
                    display_pick["로케이션"].astype(str).str.contains(search_pick, case=False, na=False) |
                    display_pick["상품코드"].astype(str).str.contains(search_pick, case=False, na=False) |
                    display_pick["점포명"].astype(str).str.contains(search_pick, case=False, na=False) |
                    display_pick["주소"].astype(str).str.contains(search_pick, case=False, na=False)
                ]

            # 정렬 조건: 출력여부 > 당/익(그다음내림차순) > 로케이션 > 상품코드(내림차순) > SIZE > PTTN > 점포명 > 주소
            # 당/익, 상품코드는 내림차순(ascending=False), 나머지는 오름차순(ascending=True) 등 조건 맞춤
            # '당/익(그다음내림차순)'이므로 당익 컬럼 내림차순, 상품코드 내림차순 적용
            display_pick = display_pick.sort_values(
                by=["출력여부", "당익", "로케이션", "상품코드", "SIZE", "PTTN", "점포명", "주소"],
                ascending=[True, False, True, False, True, True, True, True]
            ).reset_index(drop=True)

            st.dataframe(display_pick, use_container_width=True, hide_index=True, height=550)
    ...
    # --- 4단계: 세부사항 뷰 (오류 수정 완료) ---
        ...
        if row_data is not None:
            ...
            # 피킹조회(df_p) 원본 데이터가 있다면 병합하여 상세 품목/로케이션 정보를 확실히 보완
            if df_p is not None and "일련번호" in df_p.columns and "일련번호" in matching_items.columns:
                df_p_temp = df_p.copy()
                df_p_temp["serial_key"] = df_p_temp["일련번호"].astype(str).str.strip()
                
                # 피킹 파일의 세부 컬럼 우선 매핑
                p_cols_to_map = {}
                for col in ["로케이션", "DOT", "LI", "SS", "품목코드", "SIZE", "PTTN", "품목비고"]:
                    if col in df_p_temp.columns:
                        p_cols_to_map[col] = df_p_temp.groupby("serial_key")[col].first().to_dict()

                matching_items["serial_key"] = matching_items["일련번호"].astype(str).str.strip()
                for col, d_map in p_cols_to_map.items():
                    mapped_vals = matching_items["serial_key"].map(d_map)
                    if col == "로케이션":
                        matching_items["로케이션_표시"] = mapped_vals.fillna(matching_items["로케이션_표시"])
                    elif col == "DOT":
                        matching_items["디오티_표시"] = mapped_vals.fillna(matching_items["디오티_표시"])
                    elif col == "LI":
                        matching_items["LI_표시"] = mapped_vals.fillna(matching_items["LI_표시"])
                    elif col == "SS":
                        matching_items["SS_표시"] = mapped_vals.fillna(matching_items["SS_표시"])
                    elif col in ["품목코드", "상품코드"]:
                        matching_items["상품코드_표시"] = mapped_vals.fillna(matching_items["상품코드_표시"])
                    elif col == "SIZE":
                        matching_items["사이즈_표시"] = mapped_vals.fillna(matching_items["사이즈_표시"])
                    elif col == "PTTN":
                        matching_items["패턴_표시"] = mapped_vals.fillna(matching_items["패턴_표시"])
                    elif col == "품목비고":
                        matching_items["품목비고_표시"] = mapped_vals.fillna("-")

            if "상품코드_표시" not in matching_items.columns:
                matching_items["상품코드_표시"] = matching_items.get("품목코드", "-")

            if "품목비고_표시" not in matching_items.columns:
                matching_items["품목비고_표시"] = matching_items.get("품목비고", "-")
