...
def render_main_app():
    ...
    # --- 2단계: 배송 유형 선택 ---
        ...
        # 택배의 경우: 승용 선택시 (승용+화물+마케팅) 전체 / 화물 선택시 화물만
        if cat == "승용":
    ...
    # --- 3단계: 목록 현황 ---
        ...
        if sub == "택배":
            if cat == "승용":
                sub_df = df[df["배송유형"] == "택배"].copy() # 승용, 화물, 마케팅 전부 표현
            elif cat == "화물":
        ...
        if sub == "택배":
            ...
            search_kw = st.text_input("🔍 검색어 입력 (타이어구분 / 상품코드 / 패턴 / 사이즈)", "")
            if search_kw:
                sub_df = sub_df[
            ...
            grouped_df = sub_df.groupby(
                ["타이어구분_표시", "상품코드_표시", "패턴_표시", "사이즈_표시"], 
                as_index=False
            )["수량_표시"].sum()

            show_df = grouped_df.rename(columns={
                "타이어구분_표시": "타이어구분",
                "상품코드_표시": "상품코드",
                "패턴_표시": "패턴",
                "사이즈_표시": "사이즈",
        ...
        # 일반 배송 화면 레이아웃 (당착, 익착, 미배차, 내일물량)
            ...
            def highlight_vehicles(data):
                ...
                for i, key in enumerate(car_keys):
                    ...
                    prev_key = key
                    styles.iloc[i] = "background-color: #f2f4f8;" if color_flag else "background-color: #ffffff;"
                return styles
