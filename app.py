...
def process_uploaded_files(files_dict):
    ...
    if df_p is not None and "일련번호" in df_p.columns:
        ...
        # 세부사항 표시용 BC(바코드품목)는 별도 컬럼이 아니라
        # 피킹조회 파일의 PTTN 값으로 판정한다.
        BC_PATTERNS = {
            "TURANZA EV",
            "POTENZA SPORT AS",
            "DESTINATION LE3",
        ...
        if "PTTN" in df_p.columns:
            ...
            df_p["_BC_표시"] = np.where(pttn_norm.isin(BC_PATTERNS), "Y", np.nan)
            bc_map = df_p.groupby("serial_key")["_BC_표시"].first().to_dict()
        else:
            bc_map = {}
    ...
    # BC(바코드품목)는 피킹조회 PTTN 기준으로만 판정한다.
    # 피킹 파일이 없거나 PTTN을 찾지 못한 경우에는 미표시 처리한다.
